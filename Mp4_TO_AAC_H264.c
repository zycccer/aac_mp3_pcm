...
int main(int argc,char **argv){
    ...
    ret = av_bsf_alloc(bitStreamFilter,&avbsfContext);
    if(ret < 0) {
        av_strerror(ret, errors, ERROR_STRING_SIZE);
    ...
    ret = avcodec_parameters_copy(avbsfContext->par_in,ifmt_ctx->streams[video_index]->codecpar);
    if(ret < 0) {
        av_strerror(ret, errors, ERROR_STRING_SIZE);
    ...
    while (1){
        ...
        if (packet->stream_index == video_index){
            ret = av_bsf_send_packet(avbsfContext, packet); // 内部把我们传入的buf转移到自己bsf内部
            if(ret < 0) {       // 基本不会进入该逻辑
                av_strerror(ret, errors, ERROR_STRING_SIZE);
            ...
            while (1) {
                ret = av_bsf_receive_packet(avbsfContext, packet);
                if(ret != 0) {
                    break;
                }
                size_t size = fwrite(packet->data, 1, packet->size, h264_fd);
                if(size != packet->size)
                {
            ...
            size_t size = fwrite(packet->data,1,packet->size,aac_fd);
            if (size!=packet->size){
