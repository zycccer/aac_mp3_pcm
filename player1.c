...
int main(int argc,char** argv){
    ...
    // 也可以直接findbeststream  一样没啥区别    之前的代码反而好理解
    videoStream=-1;
    ...
    // initialize SWS context for software scaling  看格式了
    ...
    while (av_read_frame(pFormatCtx,&packet)>=0){
        if (packet.stream_index == videoStream){
            //3.x代码
            // 4.x decoder 需要自己设计一下而不是一行代码  ret = Decode(pCodecCtx, pFrame, packet); 见前文
